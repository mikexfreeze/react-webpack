...
import {CellMenu, LinkCell, AlertStatus, AlertLevel} from "./Cells"
...
export default class Alerts extends MainPage {
  ...
  contentHeader = () => {
    let select = true
    if(this.state.selectedRows.length > 0){select = false}
    return (
      <div className="content-header">
        <Row>
          <Col span={12}>
            {checkPermisson("",(
              <Button type="primary" className={"mr10"} onClick={this.hdlCheck} disabled={select} size={this.size}>
                <Fm id="alertMgt.batchConfirm" tagName="span" defaultMessage="批量确认" />
              </Button>
            ))}
            {checkPermisson("",(
              <Button type="primary" className={"mr10"} onClick={this.hdlClear} disabled={select} size={this.size}>
                <Fm id="alertMgt.batchEliminate" tagName="span" defaultMessage="批量消除" />
              </Button>
            ))}
