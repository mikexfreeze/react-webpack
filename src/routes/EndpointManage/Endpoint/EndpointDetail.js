...
import {Card, Divider, Spin, Button, Row, Col, Tabs} from 'antd';
...
import Map from './Map'
import {PointsList, MeasureList} from './Components'
import {AlertList, AssignmentsList, Command, History} from "routes/EndpointManage/Endpoint/Components";
        ...
        { endpoint.id ?
          <Card bordered={false}>
            <DescriptionList size="large"
              ...
              {assign}
              <Description term={this.msg['common.connectStatus']}><LinkStatus offset={badgeOffset} value={endpoint.status} /></Description>
              <Description term={this.msg['common.alertStatus']}><AlertStatus offset={badgeOffset} value={endpoint.alertStatus} /></Description>
            ...
            <Tabs defaultActiveKey="1">
              <TabPane tab={this.msg['assignment.common.associate']} key="1">
                <div style={{minHeight: 300,position: "relative"}}>
                  <AssignmentsList tenantId={this.tenantId} endpointId={this.id} location={this.props.location} />
                </div>
              </TabPane>
              <TabPane tab={this.msg['common.map']} key="2">
                <Map tenantId={this.tenantId} endpointId={this.id} />
              </TabPane>
              <TabPane tab={this.msg['common.locations']} key="3">
                <PointsList tenantId={this.tenantId} endpointId={this.id} />
              </TabPane>
              <TabPane tab={this.msg['common.measurements']} key="4">
                <MeasureList tenantId={this.tenantId} endpointId={this.id} />
              </TabPane>
              <TabPane tab={this.msg['common.alerts']} key="5">
                <AlertList tenantId={this.tenantId} endpointId={this.id} />
              </TabPane>
              <TabPane tab={this.msg['common.commands']} key="6">
                <Command token={endpoint.specToken} tenantId={this.tenantId} endpointId={this.id} />
              </TabPane>
              <TabPane tab={this.msg['common.changeLog']} key="7">
                <History tenantId={this.tenantId} endpointId={this.id} />
              </TabPane>
            </Tabs>
